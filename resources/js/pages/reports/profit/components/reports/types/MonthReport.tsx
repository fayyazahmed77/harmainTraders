...
const MonthReport: React.FC<Props> = ({ data, formatCurrency }) => {
    return (
        <>
            <TableHeader>
                <TableRow className="border-border/50 hover:bg-transparent">
                    <TableHead className="text-[9px] font-black text-text-muted uppercase h-10 w-12 text-center">S.#</TableHead>
                    <TableHead className="text-[9px] font-black text-text-muted uppercase h-10">Month</TableHead>
                    ...
                    <TableHead className="text-[9px] font-black text-text-muted uppercase h-10 text-right">Gross Profit Loss</TableHead>
                    <TableHead className="text-[9px] font-black text-text-muted uppercase h-10 text-center w-[80px]">%</TableHead>
                    <TableHead className="text-[9px] font-black text-text-muted uppercase h-10 text-right">Expense</TableHead>
                    <TableHead className="text-[9px] font-black text-text-muted uppercase h-10 text-right">Net Profit Loss</TableHead>
                    <TableHead className="text-[9px] font-black text-text-muted uppercase h-10 text-center w-[80px]">%</TableHead>
                </TableRow>
            </TableHeader>
            <TableBody>
                {data.map((row, idx) => (
                    <TableRow key={idx} className="border-border/30 hover:bg-surface-1 transition-colors group">
                        ...
                        <TableCell className="text-[10px] font-black text-text-primary uppercase">{row.month}</TableCell>
                        ...
                        <TableCell className="text-[10px] font-bold text-amber-600 dark:text-amber-400 text-right tabular-nums">{formatCurrency(row.expense || 0)}</TableCell>
                        <TableCell className={`text-[10px] font-black text-right tabular-nums ${(row.net_profit || 0) >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                            {formatCurrency(row.net_profit || 0)}
                        </TableCell>
                        <TableCell className="text-center">
                            <div className={`inline-flex items-center justify-center px-1.5 py-0.5 rounded-sm text-[9px] font-black tabular-nums border ${(row.net_margin || 0) >= 0 ? 'bg-emerald-500/10 border-emerald-600/20 text-emerald-600 dark:text-emerald-400' : 'bg-rose-500/10 border-rose-600/20 text-rose-600 dark:text-rose-400'}`}>
                                {row.net_margin || 0}%
                            </div>
                        </TableCell>
                    </TableRow>
                ...
                {data.length > 0 && (() => {
                    ...
                    const totalNetMargin = totals.revenue > 0 ? (totals.net_profit / totals.revenue) * 100 : 0;
                    ...
                    return (
                        <TableRow className="bg-surface-1/50 border-t-2 border-border font-black hover:bg-surface-1/50">
                            ...
                            <TableCell className="text-[10px] text-amber-600 dark:text-amber-400 text-right tabular-nums">{formatCurrency(totals.expense)}</TableCell>
                            <TableCell className={`text-[10px] text-right tabular-nums ${totals.net_profit >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                                {formatCurrency(totals.net_profit)}
                            </TableCell>
                            <TableCell className="text-center">
                                <div className={`inline-flex items-center justify-center px-1.5 py-0.5 rounded-sm text-[9px] font-black tabular-nums border ${totalNetMargin >= 0 ? 'bg-emerald-500/10 border-emerald-600/20 text-emerald-600 dark:text-emerald-400' : 'bg-rose-500/10 border-rose-600/20 text-rose-600 dark:text-rose-400'}`}>
