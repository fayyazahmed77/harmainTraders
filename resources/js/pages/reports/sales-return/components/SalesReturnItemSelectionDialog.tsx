...
interface SalesReturnItemSelectionDialogProps {
    open: boolean;
...
export function SalesReturnItemSelectionDialog({
    open,
    onOpenChange,
    items,
    selectedItemId,
    onSelect
}: SalesReturnItemSelectionDialogProps) {
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                        <div>
                            <DialogTitle className="text-xl font-black italic tracking-tighter uppercase flex items-center gap-2">
                                ...
                                Select <span className="text-orange-600">Product Item</span>
                            </DialogTitle>
                            <DialogDescription className="text-text-muted text-[10px] font-bold uppercase tracking-widest mt-1 opacity-60">
                                Search and select a product to analyze its return performance.
                            </DialogDescription>
                        </div>
                                ...
                                {(itemSearch === '' || 'all'.includes(itemSearch.toLowerCase())) && (
                                    <tr 
                                        className={cn(
                                            ...
                                            selectedItemId === 'ALL' && "bg-orange-500/5"
                                        )}
                                        onClick={() => {
                                            onSelect('ALL');
                                            onOpenChange(false);
                                    ...
                                    filteredItems.map((item) => (
                                        <tr 
                                            ...
                                            className={cn(
                                                ...
                                                selectedItemId === item.id.toString() && "bg-orange-500/5"
                                            )}
                                            onClick={() => {
                                    ...
                                    <tr>
                                        <td colSpan={2} className="h-[200px] text-center align-middle">
                                            <div className="flex flex-col items-center justify-center text-text-muted/40">
                                                ...
                                                <Button variant="link" onClick={() => setItemSearch('')} className="text-[10px] text-orange-500 font-black mt-2 uppercase tracking-tighter shadow-none">Clear search filters</Button>
                                            </div>
                                        </td>
                                    </tr>
                                )}
