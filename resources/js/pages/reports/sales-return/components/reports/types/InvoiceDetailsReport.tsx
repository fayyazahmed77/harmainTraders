...
export default function InvoiceDetailsReport({ data, formatCurrency }: Props) {
    ...
    // Group items by invoice (sale_id / return_id)
    ...
    return (
        <div className="space-y-3 pb-4">
            {sortedInvoices.map((inv: any, idx: number) => (
                <div key={inv.id} className="relative group/inv w-full">
                    ...
                    <div className="bg-surface-2 text-text-primary p-2 rounded-t-sm flex flex-wrap items-center justify-between gap-6 shadow-sm relative z-10 border-b border-border/50">
                        <div className="flex items-center gap-8">
                            <div className="flex flex-col">
                                ...
                                <span className="text-xl font-black italic tracking-tighter text-orange-600 dark:text-orange-500">{inv.invNo}</span>
                            </div>
                        ...
                        <div className=" px-6 py-2 rounded-sm">
                            ...
                            <span className="text-xl font-black tabular-nums tracking-tighter text-orange-600 dark:text-orange-500">{formatCurrency(inv.totalAmount)}</span>
                        </div>
                    </div>
                        ...
                        <table className="w-full">
                            ...
                            <tbody>
                                {inv.items.map((item: any, iIdx: number) => (
                                    <tr key={iIdx} className="border-b border-border/10 hover:bg-orange-500/5 transition-colors group/row">
                                        ...
                                        <td className="py-2.5 px-4 text-right text-[11px] font-black text-orange-600 dark:text-orange-500 tabular-nums">{formatCurrency(item.amount)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
