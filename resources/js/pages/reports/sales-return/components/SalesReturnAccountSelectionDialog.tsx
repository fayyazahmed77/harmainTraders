...
interface SalesReturnAccountSelectionDialogProps {
    open: boolean;
...
export function SalesReturnAccountSelectionDialog({
    open,
    onOpenChange,
    accounts,
    selectedAccountId,
    onSelect
}: SalesReturnAccountSelectionDialogProps) {
    ...
    const filteredAccounts = useMemo(() => {
        ...
        return accounts.filter(acc =>
            ...
            (acc.area_name && acc.area_name.toLowerCase().includes(search))
        );
                ...
                <DialogHeader className="p-6 pb-4 bg-surface-1/50 text-text-primary relative flex flex-col gap-0 border-b border-border/20">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                        <div>
                            <DialogTitle className="text-xl font-black italic tracking-tighter uppercase flex items-center gap-2">
                                ...
                                Select <span className="text-orange-600">Customer Account</span>
                            </DialogTitle>
                            <DialogDescription className="text-text-muted text-[10px] font-bold uppercase tracking-widest mt-1 opacity-60">
                                Search and select a customer to filter return intelligence.
                            </DialogDescription>
                        </div>
                    ...
                    <div className="relative">
                        ...
                        <Input 
                            ...
                            className="h-9 pl-9 pr-4 bg-surface-0 border-border/50 focus:ring-1 focus:ring-orange-500/50 focus:border-orange-500 rounded-sm text-[11px] transition-all text-text-primary placeholder:text-text-muted/30 font-bold"
                            value={accountSearch}
                        ...
                        <table className="w-full border-collapse">
                            ...
                            <tbody>
                                {(accountSearch === '' || 'all'.includes(accountSearch.toLowerCase())) && (
                                    <tr 
                                        className={cn(
                                            "group cursor-pointer transition-all hover:bg-orange-500/5",
                                            selectedAccountId === 'ALL' && "bg-orange-500/5"
                                        )}
                                        onClick={() => {
                                            onSelect('ALL');
                                            onOpenChange(false);
                                    ...
                                    >
                                        <td className="px-6 py-3">
                                            <Badge variant="outline" className="font-mono text-[9px] font-black bg-surface-1 border-orange-500/30 text-orange-600 dark:text-orange-400 rounded-none">
                                                ALL
                                            </Badge>
                                        </td>
                                        <td className="px-6 py-3">
                                            <div className="font-black text-[11px] text-text-secondary group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors uppercase tracking-tight">
                                    ...
                                    filteredAccounts.map((acc) => (
                                        <tr 
                                            ...
                                            className={cn(
                                                ...
                                                selectedAccountId === acc.id.toString() && "bg-orange-500/5"
                                            )}
                                            onClick={() => {
                                        ...
                                        >
                                            <td className="px-6 py-2.5">
                                                <Badge variant="outline" className="font-mono text-[9px] font-bold bg-surface-1/50 border-border/50 text-text-muted group-hover:border-orange-500/30 group-hover:text-orange-500 rounded-none">
                                                    {acc.code || "---"}
                                                </Badge>
                                            </td>
                                            <td className="px-6 py-2.5">
                                                <div className="font-bold text-[11px] text-text-primary group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors">
                                                    {acc.title}
                                                </div>
                                            </td>
                                            <td className="px-6 py-2.5 text-right">
                                                <span className="text-[10px] font-black text-text-muted/40 uppercase">
                                                    {acc.area_name || acc.area?.name || "---"}
                                                </span>
                                            </td>
                                        </tr>
                                    ))
                                    ...
                                    <tr>
                                        <td colSpan={3} className="h-[200px] text-center align-middle">
                                            <div className="flex flex-col items-center justify-center text-text-muted/40">
                                                ...
                                                <Button variant="link" onClick={() => setAccountSearch('')} className="text-[10px] text-orange-500 font-black mt-2 uppercase tracking-tighter shadow-none">Clear search filters</Button>
                                            </div>
                                        </td>
                                    </tr>
                                )}
