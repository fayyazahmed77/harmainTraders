...
interface ReceivingDetailReportViewProps {
    data: ReceivingDetail[];
}

export function ReceivingDetailReportView({ data }: ReceivingDetailReportViewProps) {
    ...
    return (
        <div className="relative -mt-6">
                ...
                <div className="overflow-x-auto custom-scrollbar">
                    <table className="w-full border-separate border-spacing-0 p-0 m-0">
                        <thead className="sticky top-0 z-20">
                            <tr className="bg-gradient-to-r from-emerald-600/90 to-teal-700/90 backdrop-blur-2xl border-b border-border/40 hover:bg-emerald-600 h-9">
                                ...
                                <th className="text-[9px] font-display font-black uppercase tracking-[0.2em] text-white m-0 border-b border-border/40 text-left align-middle h-9">Received Date</th>
                                ...
                                <TableRow key={item.id} className={cn(
                                    ...
                                    <TableCell className="py-1.5 px-2">
                                        <span className="text-[10px] font-bold text-text-muted tracking-tight">
                                            {formatSafeDate(item.date, 'dd MMM yy').toUpperCase()}
                                        </span>
                                    </TableCell>
                                    <TableCell className="py-1.5 px-2">
                                    ...
                                    <TableCell className="text-right py-1.5 px-2 text-[11px] font-mono-jet font-black text-emerald-600 dark:text-emerald-400">
            ...
            <div className="bg-surface-4 dark:bg-surface-3 p-4 px-8 mt-3 rounded-xl flex items-center justify-between border-b-2 border-emerald-500/30 shadow-sm">
                <div className="flex items-center gap-6">
                    <Scale className="h-5 w-5 text-emerald-400" />
                    <div>
                        <h5 className="text-[8px] font-display font-black text-text-muted uppercase tracking-[0.3em]">Receiving Consolidation</h5>
                        <p className="text-sm font-display font-black text-text-primary tracking-widest uppercase">Inbound Intelligence</p>
                    </div>
                </div>

                <div className="flex items-center gap-12">
                    <div className="flex flex-col items-end">
                        <span className="text-[8px] font-display font-black text-text-muted uppercase tracking-widest mb-1">Total Inbound Volume</span>
                        <p className="text-base font-mono-jet font-black tracking-tighter text-emerald-600 dark:text-emerald-400">
