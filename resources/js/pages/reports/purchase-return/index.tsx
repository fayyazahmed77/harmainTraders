...
import { RotateCcw, Package, Users, TrendingDown } from 'lucide-react';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: "Purchase Return Reports", href: "/reports/purchase-return" },
];

export default function PurchaseReturnReport({ suppliers }: Props) {
    const [dateRange, setDateRange] = useState<DateRange | undefined>({
        from: subDays(new Date(), 30),
        to: new Date(),
    });
    const [data, setData] = useState<PurchaseReturnData | null>(null);
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await axios.get(route('reports.purchase-return'), {
                params: {
            ...
            console.error("Failed to fetch purchase return data", error);
            toast.error("Failed to load purchase return report");
        } finally {
            setLoading(false);
        }
    };
    ...
    const paginatedData = useMemo(() => {
        ...
        return data.returns.slice(start, start + pagination.pageSize);
    }, [data, pagination]);

    const pageCount = data ? Math.ceil(data.returns.length / pagination.pageSize) : 0;
    ...
    return (
        <>
            <Head title="Purchase Return Reports" />
            <SidebarProvider>
                <AppSidebar />
                <SidebarInset>
                    ...
                    <div className="p-6 space-y-6 min-h-screen">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div>
                                <h1 className="text-3xl font-bold tracking-tight">Purchase Return Reports</h1>
                                <p className="text-slate-500">Track and analyze purchase returns</p>
                            </div>
                            <div className="flex items-center gap-4">
                                ...
                                <ButtonGroup>
                                    ...
                                    >
                                        <a
                                            href={route("reports.purchase-return.export.pdf", {
                                                from: dateRange?.from ? format(dateRange.from, "yyyy-MM-dd") : null,
                                                to: dateRange?.to ? format(dateRange.to, "yyyy-MM-dd") : null,
                                                supplier_id: selectedSupplier !== "all" ? selectedSupplier : null,
                                            })}
                                    ...
                                    >
                                        <a
                                            href={route("reports.purchase-return.export.excel", {
                                                from: dateRange?.from ? format(dateRange.from, "yyyy-MM-dd") : null,
                                                to: dateRange?.to ? format(dateRange.to, "yyyy-MM-dd") : null,
                                                supplier_id: selectedSupplier !== "all" ? selectedSupplier : null,
                                            })}
                        ...
                        <div className="grid gap-6 md:grid-cols-4">
                            <Card className="border-l-4 border-l-red-500 shadow-sm bg-red-100">
                                ...
                                <CardContent>
                                    ...
                                    <p className="text-xs text-muted-foreground">{data.summary.return_count} transactions</p>
                                </CardContent>
                            </Card>
                            ...
                            <Card className="border-l-4 border-l-orange-500 shadow-sm bg-orange-100">
                                ...
                                <CardContent>
                                    <div className="text-2xl font-bold text-orange-600">
                                        {formatCurrency(data.summary.average_return)}
                                    </div>
                        ...
                        <div className="grid gap-6 md:grid-cols-2">
                            <ChartCard title="Daily Return Trend">
                                <div className="h-[300px] w-full">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <AreaChart data={data.daily_trend}>
                                            <defs>
                                                <linearGradient id="colorReturnTotal" x1="0" y1="0" x2="0" y2="1">
                                            ...
                                            />
                                            <Area type="monotone" dataKey="total" stroke="#ef4444" fillOpacity={1} fill="url(#colorReturnTotal)" name="Total" />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                </div>
                            </ChartCard>

                            <ChartCard title="Top 10 Suppliers by Returns">
                                <div className="h-[300px] w-full">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={data.top_suppliers} layout="vertical">
                                            ...
                                            <Tooltip
                                                ...
                                                contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}
                                            />
                                            <Bar dataKey="total" fill="#ef4444" radius={[0, 4, 4, 0]} name="Total" barSize={20} />
                                        </BarChart>
                                    </ResponsiveContainer>
                                </div>
                            </ChartCard>
                        ...
                        <Card className="shadow-sm">
                            <CardHeader>
                                ...
                                <CardDescription>Detailed purchase return records</CardDescription>
                                <div className="mt-4">
