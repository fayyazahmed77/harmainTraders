...
interface PurchaseReportSectionDialogProps {
    open: boolean;
...
export function PurchaseReportSectionDialog({ 
    open, 
    onOpenChange, 
    onSelect, 
    currentReportId 
}: PurchaseReportSectionDialogProps) {
    return (
            ...
            <DialogContent className="max-w-2xl bg-surface-1/95 backdrop-blur-xl border-border/40 p-0 overflow-hidden rounded-sm">
                <DialogHeader className="p-6 border-b border-border/10 bg-surface-1">
                    <DialogTitle className="text-xl font-black text-text-primary uppercase tracking-tighter italic">
                        Select <span className="text-emerald-500">Procurement Module</span>
                    </DialogTitle>
                    <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest mt-1">Choose the dimension of analysis for your purchase data</p>
                </DialogHeader>
                            ...
                            >
                                <div className={cn(
                                    ...
                                    isActive 
                                        ...
                                        : "bg-surface-1 border-border/10 text-text-muted group-hover:text-emerald-500 group-hover:border-emerald-500/20"
                                )}>
                                ...
                                <div className="flex flex-col gap-1 min-w-0">
                                    <span className={cn(
                                        ...
                                        isActive ? "text-emerald-500" : "text-text-primary group-hover:text-emerald-500"
                                    )}>
                                        {report.title}
                                    </span>
