...
import { DollarSign, Printer, FileText, FileSpreadsheet, Package, Users, Calendar, TrendingUp, List, ClipboardList, BarChart3, Wallet, ArrowUpDown } from 'lucide-react';
    ...
    switch (reportType) {
        case 'bill':
            return [
                ...
                { value: 'paid_desc', label: 'Paid Amount: High to Low' },
                { value: 'paid_asc', label: 'Paid Amount: Low to High' },
            ];
        case 'date':
            return [
        ...
        case 'month':
            return [
                ...
                { value: 'paid_desc', label: 'Paid Amount: High to Low' },
                { value: 'paid_asc', label: 'Paid Amount: Low to High' },
                { value: 'balance_desc', label: 'Balance Payable: High to Low' },
                { value: 'balance_asc', label: 'Balance Payable: Low to High' },
            ];
        case 'payment':
            return [
                { value: 'default', label: 'Default (Supplier A-Z)' },
                { value: 'name_asc', label: 'Supplier Name: A to Z' },
                { value: 'name_desc', label: 'Supplier Name: Z to A' },
                { value: 'area_name_asc', label: 'Area Name: A to Z' },
                { value: 'area_name_desc', label: 'Area Name: Z to A' },
                { value: 'total_purchase_desc', label: 'Total Purchase: High to Low' },
                { value: 'total_purchase_asc', label: 'Total Purchase: Low to High' },
                { value: 'total_payment_desc', label: 'Total Payment: High to Low' },
                { value: 'total_payment_asc', label: 'Total Payment: Low to High' },
                { value: 'balance_desc', label: 'Current Balance: High to Low' },
                { value: 'balance_asc', label: 'Current Balance: Low to High' },
            ];
        default:
            return baseOptions;
    }
...
export function PurchaseReportView({ data, type, criteria, onExport }: PurchaseReportViewProps) {
    ...
    const totals = data.reduce((acc, row) => ({
        amount: acc.amount + (Number(row.amount) || Number(row.total_amount) || Number(row.net_total) || Number(row.total_purchase) || Number(row.net_amount) || 0),
        ...
        return dataCopy.sort((a, b) => {
            ...
            switch (sortBy) {
                ...
                case 'total_purchase_desc':
                    return (Number(b.total_purchase) || 0) - (Number(a.total_purchase) || 0);
                case 'total_purchase_asc':
                    return (Number(a.total_purchase) || 0) - (Number(b.total_purchase) || 0);
                case 'total_payment_desc':
                    return (Number(b.total_payment) || 0) - (Number(a.total_payment) || 0);
                case 'total_payment_asc':
                    return (Number(a.total_payment) || 0) - (Number(b.total_payment) || 0);
                case 'balance_desc':
                    return (Number(b.balance) || 0) - (Number(a.balance) || 0);
                case 'balance_asc':
                    return (Number(a.balance) || 0) - (Number(b.balance) || 0);
                case 'date_desc':
                    valA = String(a.date || a.month_key || '0');
                    valB = String(b.date || b.month_key || '0');
                ...
                case 'product_name_desc':
                    ...
                    return valB.localeCompare(valA, undefined, { numeric: true, sensitivity: 'base' });
                case 'area_name_asc':
                    valA = String(a.area_name || '').toLowerCase();
                    valB = String(b.area_name || '').toLowerCase();
                    return valA.localeCompare(valB, undefined, { numeric: true, sensitivity: 'base' });
                case 'area_name_desc':
                    valA = String(a.area_name || '').toLowerCase();
                    valB = String(b.area_name || '').toLowerCase();
                    ...
                    <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto justify-end">
                        ...
                        <Select value={sortBy} onValueChange={setSortBy}>
                            <SelectTrigger className="h-9 w-[190px] rounded-none border-border/50 bg-surface-1 hover:bg-surface-2 text-[10px] font-black uppercase tracking-tight focus:ring-0 focus:ring-offset-0 flex items-center gap-2">
                                <ArrowUpDown className="h-3.5 w-3.5 text-emerald-500 shrink-0" />
