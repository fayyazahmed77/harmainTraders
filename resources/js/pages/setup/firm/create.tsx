...
export default function FirmCreate() {
  ...
  const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: "Add Firm", href: "/firms/create" },
  ];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (date) {
      (finalForm as any).date = date.toLocaleDateString("en-GB", {
        day: "2-digit",
        month: "short",
        year: "numeric",
      });
    ...
    post("/firms", {
      forceFormData: true,
      onSuccess: () => {
        toast.success("Firm added successfully", { description: "New firm has been saved." });
      },
          ...
          <div className="flex-1 w-full h-full overflow-y-auto custom-scrollbar text-zinc-900 dark:text-zinc-100 pb-20">
            <div className="max-w-[1200px] mx-auto p-4 md:p-8 space-y-10">
              ...
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                <Heading
                  title="Add Firm"
                  description="Add a new firm with branding and business details"
                />
              </motion.div>
