                                ...
                                `}>
                                    <div className="flex items-center gap-4 flex-1">
                                        <Link
                                            ...
                                            className="group p-2.5 rounded-lg bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:bg-orange-500 hover:text-white transition-all active:scale-90"
                                        >
                                            <Icons.ArrowLeft size={18} className="transition-transform group-hover:-translate-x-0.5" />
                                        </Link>
                                        <div className="flex-1 min-w-0 max-w-md">
                                            <TechLabel label="Security Clearance Mapping" icon={Icons.Shield}>
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div className="hidden lg:flex flex-col items-end mr-4">
                                            <span className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em]">Active Matrix</span>
                                            <span className="text-xs font-mono font-bold text-orange-500 uppercase">{data.permissions.length} Nodes Assigned</span>
                                        </div>
                                        <Button
                                            type="submit"
                                            disabled={processing}
                                        ...
                                        >
                                            {processing ? (
                                                ...
                                                <Icons.Zap size={16} className="text-orange-400 dark:text-orange-200" />
                                            )}
                                            {processing ? 'Propagating...' : 'Commit Changes'}
                                        </Button>
                                    </div>
                                </div>
                            </div>

                            <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1 min-h-0 overflow-hidden">
                                {/* Navigation Sidebar */}
                                <div className="lg:col-span-3 space-y-6 lg:sticky lg:top-28 self-start">
                                    <div className={`p-4 ${CARD_BASE} rounded-sm border-l-4 border-zinc-900 dark:border-zinc-100 bg-zinc-50/50 dark:bg-zinc-900/30`}>
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="flex flex-col gap-4 relative z-10">
                                                                        <div className="flex items-center justify-between">
                                                                            ...
                                                                            `}>
                                                                                {isSelected ? <Icons.LockOpen size={12} strokeWidth={3} /> : <Icons.Lock size={12} className="text-zinc-400" />}
                                                                            </div>
                                                                            <span className={`text-[8px] font-black tracking-widest ${isSelected ? 'text-orange-400' : 'text-zinc-300 dark:text-zinc-600'}`}>
                                                                                {isSelected ? 'ACCESS_GRANTED' : 'RESTRICTED'}
                                                                            </span>
                                                                        </div>
