...
export default function PaymentVoucher({ accounts, paymentAccounts, messageLines = [], firms = [] }: Props) {
  ...
  router.post('/payment/store', finalPayload, {
    ...
    onError: (errors) => {
      console.error("Payment store error:", errors);
      toast.error("Failed to save payment. Please check error messages.");
    },
  ...
  // Live calculation: In Multi-Method mode, sync Primary Disbursement (amount) with sum of split payment amounts
  useEffect(() => {
    if (isMultiPayment) {
    ...
    const newSplits = splitPayments.map((row) => {
      if (row.payment_method === 'Cheque' && row.payment_account_id && !row.cheque_no) {
        ...
        if (cheques && cheques.length > 0) {
          ...
          const found = cheques.find((c: any) => {
            const val = typeof c === 'string' ? c : (c?.value || c?.cheque_no);
            return !usedCheques.includes(val);
          }) || cheques[0];
          const nextVal = typeof found === 'string' ? found : (found?.value || found?.cheque_no || "");
          modified = true;
          return { ...row, cheque_no: nextVal };
        }
      }
      return row;
    });
          ...
          <div className="flex-1 min-h-0 flex flex-col md:flex-row gap-5 md:overflow-hidden">
            ...
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="flex-1 min-h-0 flex flex-col md:overflow-hidden">
              <AnimatePresence mode="wait">
                {!isMultiPayment ? (
                  <motion.div key="registry-manifest" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} transition={{ duration: 0.2 }} className="flex-1 min-h-0 flex flex-col">
                        ...
                        <table className="w-full border-separate border-spacing-0">
                          <thead className="sticky top-0 z-10">
                            <tr className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md">
                              ...
                              <th className="px-4 py-1 text-left border-b border-zinc-100 dark:border-zinc-800 text-[10px] font-black uppercase tracking-widest text-zinc-700 dark:text-zinc-400">Invoice No</th>
                              <th className="px-4 py-1 text-left border-b border-zinc-100 dark:border-zinc-800 text-[10px] font-black uppercase tracking-widest text-zinc-700 dark:text-zinc-400">Type</th>
                              <th className="px-4 py-1 text-left border-b border-zinc-100 dark:border-zinc-800 text-[10px] font-black uppercase tracking-widest text-zinc-700 dark:text-zinc-400">Date</th>
                              <th className="px-4 py-1 text-right border-b border-zinc-100 dark:border-zinc-800 text-[10px] font-black uppercase tracking-widest text-zinc-700 dark:text-zinc-400 text-rose-500">Discount</th>
                              <th className="px-4 py-1 text-right border-b border-zinc-100 dark:border-zinc-800 text-[10px] font-black uppercase tracking-widest text-zinc-700 dark:text-zinc-400 text-rose-500">Returns</th>
                              <th className="px-4 py-1 text-right border-b border-zinc-100 dark:border-zinc-800 text-[10px] font-black uppercase tracking-widest text-zinc-700 dark:text-zinc-400">Remaining</th>
                              <th className="px-4 py-1 text-right border-b border-zinc-100 dark:border-zinc-800 text-[10px] font-black uppercase tracking-widest text-zinc-700 dark:text-zinc-400 w-40">Allocation</th>
                            </tr>
                          </thead>
                              ...
                              <tr key={row.id} className="bg-zinc-50 dark:bg-zinc-900/30 rounded-md group">
                                <td className={`p-1  group-${t.borderHover} transition-all rounded-l-md bg-white dark:bg-zinc-900 shadow-sm`}>
                                  <SearchableAccountSelector
                                    ...
                                    placeholder="Account..."
                                    className="h-8.5 text-xs font-mono px-2 bg-zinc-50 dark:bg-zinc-800/50"
                                  />
                                </td>
