...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: "New Return", href: "/purchase-return/create" },
];
...
interface RowData {
    ...
    bonus_full: number;
    bonus_pcs: number;
    sold_full?: number; // Original purchased full
    sold_pcs?: number; // Original purchased pcs
    rate: number;
    taxPercent: number;
    discPercent: number;
    trade_price: number;
...
export default function PurchaseReturnCreatePage({ items, accounts, salemans, purchase }: { items: Item[]; accounts: Account[]; salemans: Saleman[]; purchase?: any }) {
    // Header / meta fields
    ...
    const [rows, setRows] = useState<RowData[]>(() => {
        if (purchase && purchase.items) {
            return purchase.items.map((purchaseItem: any) => {
                const item = purchaseItem.item;
                const baseRate = toNumber(item.trade_price ?? 0);
                ...
                return {
                    ...
                    bonus_full: 0,
                    bonus_pcs: 0,
                    sold_full: toNumber(purchaseItem.qty_carton),
                    sold_pcs: toNumber(purchaseItem.qty_pcs),
                    rate: toNumber(purchaseItem.trade_price), // Use price from purchase
                    taxPercent: toNumber(item.gst_percent ?? 0),
                    discPercent: toNumber(item.discount ?? 0),
                    trade_price: toNumber(purchaseItem.trade_price),
                    amount: 0,
                };
            });
        }
    ...
    // Initialize state from purchase if present
    React.useEffect(() => {
        if (purchase) {
            setOriginalInvoice(purchase.invoice);
            setSalesman(purchase.salesman_id);

            // Find account option
            const accountOption = accountTypeOptions.find(opt => opt.value === purchase.supplier_id);
            if (accountOption) {
                setAccountType(accountOption);
                const selectedAccount = accounts.find((a) => a.id === purchase.supplier_id);
                if (selectedAccount) {
    ...
    }, [purchase, accountTypeOptions, accounts]);
    ...
    const loadAllItems = () => {
        const allItemRows: RowData[] = items.map((it) => {
            const baseRate = toNumber(it.trade_price ?? 0);
    ...
    const handleSelectItem = (rowId: number, itemId: number) => {
        ...
        const baseRate = toNumber(selected.trade_price ?? 0);
    ...
    const handleSave = async () => {
        const payload = {
            ...
            invoice: invoiceNo,
            original_invoice: String(originalInvoice || ""),
        ...
        router.post("/purchase-return", payload, {
            ...
            onError: (errors) => {
                ...
                alert("Failed to save purchase return. Check console for details.");
            }
        });
    };
                            ...
                            <Select
                                ...
                                onValueChange={(value) => {
                                    ...
                                    if (selectedAccount) {
                                        ...
                                        // Fetch supplier's purchased items
                                        fetch(`/purchase-return/supplier/${id}/purchased-items`)
                                            .then(res => res.json())
                                            .then(data => {
                                                if (data && Array.isArray(data)) {
                                                    ...
                                                    const purchasedRows: RowData[] = data.map((purchasedItem: any) => {
                                                        ...
                                                        return {
                                                            ...
                                                            taxPercent: toNumber(item.gst_percent ?? 0),
                                                            discPercent: toNumber(item.discount ?? 0),
                                                            trade_price: toNumber(purchasedItem.last_trade_price ?? item.trade_price ?? baseRate),
                                                            amount: 0,
                                                        };
                                                    });
                        ...
                        <div className="col-span-9">
                            <Card className="p-0 overflow-hidden gap-0">
                                ...
                                <div className="grid grid-cols-12 bg-gray-100 p-2 text-xs font-semibold border-b sticky top-0 z-10">
                                    ...
                                    <div className="col-span-1 text-center bg-yellow-100">Purch Ctn</div>
                                    <div className="col-span-1 text-center bg-yellow-100">Purch Pcs</div>
                                    ...
                                    <>
                                        ...
                                        <div className="grid grid-cols-4 gap-2">
                                            ...
                                            {(() => {
                                                ...
                                                // For Purchase Return, we are REMOVING from stock, so remaining stock decreases
                            ...
                            <Card className="p-4 space-y-3 sticky top-[120px] gap-0">
                                ...
                                {purchase && (
                                    <div className="bg-yellow-50 p-2 rounded border border-yellow-200">
                                        ...
                                        <div className="text-lg font-bold text-yellow-900">{toNumber(purchase.net_total).toFixed(2)}</div>
                                    </div>
                                )}
