...
export function DataTable({ data }: DataTableProps) {
    ...
    const handleUpdate = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        router.put(
            ...
            {
                onSuccess: () => {
                    toast.success("Account type updated", { description: "Account type has been updated successfully." });
    ...
    const handleDelete = () => {
        ...
        router.delete(`/account-types/${selectedAccountType.id}`, {
            onSuccess: () => {
                toast.success("Account type deleted", { description: "Account type has been removed successfully." });
    ...
    const columns: ColumnDef<AccountType>[] = [
        ...
        },
        {
            ...
            header: "Created By",
            cell: ({ row }) => (
                <span className="text-sm font-medium">{row.original.created_by_name || "System"}</span>
            ),
        },
        {
            id: "actions",
    ...
    return (
        <div className="w-full space-y-6">
            {/* Table */}
            <div className="rounded-md border border-zinc-200 dark:border-zinc-800/50 bg-white/50 dark:bg-zinc-900/50 overflow-hidden shadow-sm mb-2">
                <Table>
                                ...
                                {headerGroup.headers.map((header) => (
                                    <TableHead key={header.id} className="h-12 px-6">
                                        <div
                                            ...
                                            className={cn(
                                                "flex items-center gap-2 text-[12px] font-black uppercase tracking-[0.2em]  transition-colors",
                                ...
                                <TableRow>
                                    <TableCell colSpan={columns.length} className="h-40 text-center">
                                        <div className="flex flex-col items-center justify-center space-y-3 opacity-40">
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-[0.3em]">No account types found</span>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            )}
                ...
                <DialogContent className="max-w-xl rounded-2xl border-zinc-200 dark:border-zinc-800 bg-white/90 dark:bg-zinc-950/90 backdrop-blur-2xl">
                    <DialogHeader className="border-b border-zinc-100 dark:border-zinc-800 pb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <DialogTitle className="text-lg font-black uppercase tracking-widest leading-none">Edit Account Type</DialogTitle>
                                <DialogDescription className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mt-1">Update the details of this account type</DialogDescription>
                            </div>
                        </div>
                    </DialogHeader>
                ...
                <DialogContent className="max-w-md rounded-2xl border-rose-100 dark:border-rose-900/30 bg-white/90 dark:bg-zinc-950/90 backdrop-blur-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-zinc-500 text-xs font-bold uppercase tracking-tight py-4">
                            Are you sure you want to delete <span className="text-zinc-900 dark:text-zinc-100 font-black">{selectedAccountType?.name}</span>? This action cannot be undone.
                        </DialogDescription>
                    </DialogHeader>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 px-2 mb-2">
                <div className="text-[10px] font-black uppercase tracking-widest text-zinc-400 px-4 py-2 bg-zinc-100/50 dark:bg-zinc-800/50 rounded-xl">
                    Account type Page {table.getState().pagination.pageIndex + 1} OF {table.getPageCount() || 1}
                </div>
