...
<table style="border: 1px solid #000;">
    <thead>
        <tr>
            ...
            <th style="border: 1px solid #000;">Account / Item / Month</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td style="border: 0.5px solid #eee; padding: 2px 4px;">
                <div style="font-size: 8px; font-weight: bold; text-transform: uppercase;">{{ $row['account_name'] }}</div>
                <div style="font-size: 7px; color: #666; text-transform: uppercase;">{{ $row['item_name'] }} ({{ $row['month_name'] }})</div>
            </td>
            <td style="text-align: right; font-size: 8px; border: 0.5px solid #eee;">{{ number_format($row['qty_f'], 0) }}</td>
            <td style="text-align: right; font-size: 8px; border: 0.5px solid #eee;">{{ number_format($row['qty_p'], 0) }}</td>
    ...
    <tfoot>
        <tr style="border-top: 2px solid #000; font-weight: 900;">
            ...
            <td class="text-right" style="font-size: 9px; padding: 6px;">{{ number_format(collect($data)->sum('qty_f'), 0) }}</td>
            <td class="text-right" style="font-size: 9px; padding: 6px;">{{ number_format(collect($data)->sum('qty_p'), 0) }}</td>
